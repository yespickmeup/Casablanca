 ...
 */
package POS.addtl_cashins;

import POS.Main;
import POS.cashout.S1_cashout.to_cashout;
 ...
 * @author i1
 */
public class Dlg_addtl_cashins extends javax.swing.JDialog {
...
//</editor-fold>
    ...
    private Dlg_addtl_cashins(java.awt.Frame parent, boolean modal) {
        super(parent, modal);
        initComponents();
        myInit();
    }

    private Dlg_addtl_cashins(java.awt.Dialog parent, boolean modal) {
        super(parent, modal);
        initComponents();
        myInit();
    }

    public Dlg_addtl_cashins() {
        super();
        initComponents();
        myInit();

    }
    private Dlg_addtl_cashins myRef;

    private void setThisRef(Dlg_addtl_cashins myRef) {
        this.myRef = myRef;
    }
    private static java.util.Map<Object, Dlg_addtl_cashins> dialogContainer = new java.util.HashMap();
    ...
    public static Dlg_addtl_cashins create(java.awt.Window parent, boolean modal) {
    ...
    public static Dlg_addtl_cashins create(java.awt.Window parent, java.awt.Dialog.ModalityType modalType) {

        if (parent instanceof java.awt.Frame) {

            Dlg_addtl_cashins dialog = dialogContainer.get(parent);

            if (dialog == null) {
                dialog = new Dlg_addtl_cashins((java.awt.Frame) parent, false);
        ...
        if (parent instanceof java.awt.Dialog) {
            ...
            if (dialog == null) {
                dialog = new Dlg_addtl_cashins((java.awt.Dialog) parent, false);
                dialog.setModalityType(modalType);
                dialogContainer.put(parent, dialog);
                java.util.logging.Logger.getAnonymousLogger().
                        log(Level.INFO, "instances: {0}", dialogContainer.size());
    ...
    public static void main(String args[]) {
        ...
        Dlg_addtl_cashins dialog = Dlg_addtl_cashins.create(new javax.swing.JFrame(), true);
...
//                btn_0.doClick();
    ...
    private void loadData_expenses(List<S1_addtl_cashins.to_cashout> acc) {
    ...
    public static class TblexpensesModel extends AbstractTableAdapter {
        ...
        public Object getValueAt(int row, int col) {
            S1_addtl_cashins.to_cashout tt = (S1_addtl_cashins.to_cashout) getRow(row);
            switch (col) {
            case 0:
                return tt.id;
            case 1:
...
//          JOptionPane.showMessageDialog(null, user_name);
        ...
        loadData_expenses(S1_addtl_cashins.ret_data(date, user_name));
    }
...
//            return;
//        }
        S1_addtl_cashins.to_cashout to = new S1_addtl_cashins.to_cashout(id, date_added, user_name, amount, purpose, session);
        S1_addtl_cashins.add_data(to);
        data_cols();
        clear();
    }
    ...
    private void select() {
        ...
        final S1_addtl_cashins.to_cashout to = (S1_addtl_cashins.to_cashout) tbl_expenses_ALM.get(tbl_expenses.convertRowIndexToModel(idx));
        Date d = new Date();
        try {
            d = DateType.sf.parse(to.date_added);
        } catch (ParseException ex) {
            Logger.getLogger(Dlg_addtl_cashins.class.getName()).
...
//            return;
//        }
        S1_addtl_cashins.to_cashout to1 = new S1_addtl_cashins.to_cashout(to.id, date_added, user_name, amount, purpose, 0);
        S1_addtl_cashins.edit_data(to1);
        data_cols();
        clear();
    }
    ...
    private void delete() {
        ...
        final S1_addtl_cashins.to_cashout to = (S1_addtl_cashins.to_cashout) tbl_expenses_ALM.get(tbl_expenses.convertRowIndexToModel(idx));
        S1_addtl_cashins.to_cashout to1 = new S1_addtl_cashins.to_cashout(to.id, "", "", "", "", 0);
        S1_addtl_cashins.delete_data(to1);
        clear();
    }
