...
//                btn_0.doClick();
    ...
    private void loadData_expenses(List<to_cashout> acc) {
    ...
    public static class TblexpensesModel extends AbstractTableAdapter {
        ...
        public Object getValueAt(int row, int col) {
            to_cashout tt = (to_cashout) getRow(row);
            switch (col) {
            case 0:
                return tt.id;
            case 1:
...
//          JOptionPane.showMessageDialog(null, user_name);
        ...
        loadData_expenses(S1_cashout.ret_data(date, user_name));
    }
...
//            return;
//        }
        to_cashout to = new to_cashout(id, date_added, user_name, amount, purpose, session);
        S1_cashout.add_data(to);
        data_cols();
        clear();
    }
    ...
    private void select() {
        ...
        final to_cashout to = (to_cashout) tbl_expenses_ALM.get(tbl_expenses.convertRowIndexToModel(idx));
        Date d = new Date();
        try {
            d = DateType.sf.parse(to.date_added);
        } catch (ParseException ex) {
            Logger.getLogger(Dlg_cashout.class.getName()).
...
//            return;
//        }
        to_cashout to1 = new to_cashout(to.id, date_added, user_name, amount, purpose, 0);
        S1_cashout.edit_data(to1);
        data_cols();
        clear();
    }
    ...
    private void delete() {
        ...
        final to_cashout to = (to_cashout) tbl_expenses_ALM.get(tbl_expenses.convertRowIndexToModel(idx));
        to_cashout to1 = new to_cashout(to.id, "", "", "", "", 0);
        S1_cashout.delete_data(to1);
        clear();
    }
