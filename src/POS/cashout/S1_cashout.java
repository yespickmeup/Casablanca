...
public class S1_cashout {
    ...
    public static void add_data(to_cashout to_expenses) {
        try {
            ...
            String s0 = "insert into " + MyDB.getNames() + ".cashouts("
                    + "date_added"
                    + ",user_name"
                    + ",amount"
                    + ",purpose"
            ...
            stmt.execute();
            Lg.s(S1_cashout.class, "Successfully Added");
        } catch (Exception e) {
    ...
    public static void edit_data(to_cashout to_expenses) {
        try {
            Connection conn = MyConnection.connect();
            String s0 = "update " + MyDB.getNames() + ".cashouts set "
            ...
            PreparedStatement stmt = conn.prepareStatement(s0);
            stmt.execute();
            Lg.s(S1_cashout.class, "Successfully Updated");
        } catch (Exception e) {
            throw new RuntimeException(e);
        } finally {
            MyConnection.close();
    ...
    public static void delete_data(to_cashout to_expenses) {
        try {
            Connection conn = MyConnection.connect();
            String s0 = "delete from " + MyDB.getNames() + ".cashouts where "
                    + " id ='" + to_expenses.id + "' "
                    + " ";

            PreparedStatement stmt = conn.prepareStatement(s0);
            stmt.execute();
            Lg.s(S1_cashout.class, "Successfully Deleted");
        } catch (Exception e) {
            throw new RuntimeException(e);
        } finally {
            MyConnection.close();
